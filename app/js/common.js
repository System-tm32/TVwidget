	...
	socket.onmessage = function(event) {
		...
		if (!window.isMobile()) {
			 gyro = JSON.parse(event.data);
		  		...
		  		//console.log("gamma: "+ gyro.gamma +"| beta"+  gyro.beta + "| alpha:" + gyro.alpha );
		  	
		  		$(".control").css("transform", "rotateY(" + (gyro.gamma - gamma) + "deg) rotateX(" + (-1)*(gyro.beta - beta) + "deg) rotateZ(" + (-1)*(gyro.alpha - alpha) + "deg)");
